...
def launch_operation_examples(tvb_client_instance):
    ...
    tvb_client_instance.launch_operation({
        "type": "ZIPConnectivity",
        "path": file_path,
    ...
    print("Importing a surface from ZIP...")
    ...
    print("Get the result of surface import...")
    surface_dto = tvb_client_instance.get_operation_results(operation_gid)[0]

    print("Importing a region mapping...")
    ...
    print("Get the result of region mapping import...")
    ...
    print("1.Download and load the region mapping with all its references...")
    ...
    print("1.This region mapping is linked to a connectivity with GID={} and number_of_regions={}".format(
        region_mapping_complete.connectivity.gid, region_mapping_complete.connectivity.number_of_regions))

    print("2.Download and load the region mapping with only GIDs for its references...")
    region_mapping_with_links = tvb_client_instance.load_datatype_with_links(region_mapping_dto.gid,
                                                                             tvb_client_instance.temp_folder)
    print("2.This region mapping is linked to a connectivity with GID={}".format(
        region_mapping_with_links.connectivity.gid))

    print("3.Only download the region mapping on client machine...")
    region_mapping_path = tvb_client_instance.retrieve_datatype(region_mapping_dto.gid, tvb_client_instance.temp_folder)

    print("3.Load the region mapping that was already downloaded on client machine...")
    local_region_mapping_with_links = tvb_client_instance.load_datatype_from_file(region_mapping_path)
    print("3.This region mapping is linked to a connectivity with GID={}".format(
    ...
    print("Get the result of BCT...")
    bct_dto = tvb_client_instance.get_operation_results(operation_gid)[0]
    print("The resulted BCT has GID={}".format(bct_dto.gid))
